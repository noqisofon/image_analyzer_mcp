...
def find_sub_image_boxes(
    ...
    Parameters:
        ...
        padding: 離れたパーツ（頭と胴体など）を同一オブジェクトとしてまとめる膨張サイズ(px)。ドット絵タイルセットや小物が密着したスプライトシートでは padding が大きすぎると隣接オブジェクト同士が連結して巨大な1つのボックスになってしまうため、padding=0 または 1 の指定を推奨します。
        ...
        merged_threshold_ratio: 中央値に対する面積比の閾値（デフォルト20.0）。これを超えるボックスに warning='possible_merged_components' を付与します。

    警告フラグ (warning) の仕様と注意点:
        - 'entire_image_or_wrong_bg': ボックス面積が画像全体の 90% 以上を占める場合に付与されます。背景色の指定が合っていないか、または密着したスプライトシートで padding が大きすぎて全要素が1つに連結してしまった可能性があります（padding=0 または 1 をお試しください）。
        - 'possible_merged_components': 中央値比が merged_threshold_ratio を超える場合に付与されます。密集したオブジェクトの融合を示唆しますが、周囲の小物に対して圧倒的に巨大な正常の単体スプライト（大型ボスや建造物など）の場合にも付与されることがあります。
        - 閾値調整のトレードオフ: merged_threshold_ratio を 4.0〜5.0 などに下げると数個の小物が融合した小規模な融合ボックスも拾いやすくなりますが、同時に正常な中・大型スプライトにも警告が付きやすくなります（面積比のみでは両者を完全に分離することは困難です）。
        - 警告なし ≠ 完全分離済み: 警告が付いていないボックスであっても、数個の小物の融合（面積比が閾値未満のもの）が存在する可能性があります。
        - 密集シートの確認方法: 密集した画像では preview_boxes や view_region(box=..., scale=2.0〜4.0) での拡大目視確認、または等間隔グリッドの場合は get_grid_boxes の併用を推奨します。
    """
    img = load_image_safely(image_path)
    if img is None:
        raise ValueError(f"画像を読み込めませんでした: {image_path}")
