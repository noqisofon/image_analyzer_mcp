...
- **`padding`（結合パディング）の推奨**:
  - デフォルトは `2` です。キャラクターの頭と胴体など離れたパーツを1つにまとめるのに有効です。
  - ただし、小物が隙間なく密着しているドット絵シートでは、`padding=2` だと隣接要素同士がすべて連結して画像全体が1つの巨大ボックスになってしまう場合があります。密着スプライトシートでは **`padding=0` または `1`** を指定してください。
- **融合警告フラグ (`warning`)**:
  - `entire_image_or_wrong_bg`: ボックス面積が画像全体の 90% 以上を占めている場合に付与されます。背景色の指定不一致か、または密着したシートで `padding` が大きすぎて全要素が連結した可能性があります（`padding=0` または `1` をお試しください）。中央値計算からは自動的に除外されます。
  - `possible_merged_components`: 有効ボックスが 5 個以上ある場合、中央値の `merged_threshold_ratio`（既定 20 倍）を超えるボックスに付与されます。密集したオブジェクトの融合を示唆しますが、周囲の小物に対して圧倒的に巨大な正常の単体スプライト（大型ボスや建造物など）の場合にも警告が付くことがあります。
  - **閾値調整のトレードオフ**: `merged_threshold_ratio` を 4.0〜5.0 などに下げると数個の小物が融合した小規模な融合ボックスも拾いやすくなりますが、同時に正常な中・大型スプライト（水晶の塊や玉座など）にも警告が付きやすくなります（面積比のみでは両者を完全に分離することは困難です）。
  - **重要: 警告なし ＝ 完全に単一オブジェクトに分離されている保証ではありません**。数個の小物が融合している場合（面積比が閾値未満のもの）は面積比だけでは検知できません。密集したスプライトシートでは `preview_boxes` での目視確認や、等間隔タイルの場合は `get_grid_boxes` の併用を推奨します。
- **`preview_boxes` と `view_region` の活用**:
  - 通常ボックスは緑枠線（`#idx`）、警告付きボックスは**太枠シアン線（`!#idx`）**で描画され、一目で問題箇所を把握できます。
  - 不透明な帯ではなく、黒縁取り＋白文字による 2 パス描画を採用しているため、文字が重なっても背後のドット絵を覆い隠しません。
  - 密着した領域ではラベル同士が一部重なることがあるため、怪しい箇所は **`view_region(box=..., scale=2.0〜4.0)`** で拡大して確認する運用が効果的です。
